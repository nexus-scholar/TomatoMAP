...
def _build_detectron2_cfg(args: argparse.Namespace, train_name: str, val_name: str):
    ...
    # --- PROPOSED FIXES for TomatoMAP Fine-Grained Segmentation ---

    # 1. Class Imbalance Handling
    cfg.DATALOADER.SAMPLER_TRAIN = "RepeatFactorTrainingSampler"
    cfg.DATALOADER.REPEAT_THRESHOLD = 0.05  # oversample instances representing < 5% of dataset

    # 2. Small Object Optimizations: Anchors
    # The default for FPN is typically [32, 64, 128, 256, 512]. We shift this down for small tomatoes (2mm, 4mm).
    cfg.MODEL.ANCHOR_GENERATOR.SIZES = [[16], [32], [64], [128], [256]]

    # 3. Increase maximum allowed detections per image (dense clusters in crops)
    cfg.TEST.DETECTIONS_PER_IMAGE = 300

    # 4. Multi-Scale Training / Data Augmentation base config
...
def _seg_train_detectron2(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    class ImprovedTrainer(DefaultTrainer):
        @classmethod
...
def _seg_train(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if "coco-instancesegmentation" not in model_name:
        raise RuntimeError("Improved engine currently supports Detectron2 COCO-InstanceSegmentation models only.")
...
def _seg_eval(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if backend == "detectron2" or "coco-instancesegmentation" in str(args.model_path).lower():
